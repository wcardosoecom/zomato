...
def best_5_cities (df1, coluna):
    """ Essa função cria um gráfico de barras com as 5 cidades com melhor nota média 
    ou a melhor média de avaliações recebidas por país.
    coluna = 'aggregate_rating' (nota média) ou 'votes' (avaliações recebidas)
    Input = Dataframe
    Output = Gráfico de barras
    """
    nozero = df1[coluna] != 0
    city_mean_best = (df1.loc[nozero, ['city', 'country', coluna]]
...
def city_cuisines (df1):
    """ Essa função cria um gráfico de barras com as 25 cidades com maior variedade culinária
    Input = Dataframe
    Output = Gráfico de barras
    """
...
def city_delivery (df1):
    """ Essa função cria um gráfico de barras com as 25 cidades com mais restaurantes que fazem entrega
    Input = Dataframe
    Output = Gráfico de barras
    """
...
with st.container():# gráfico de barras - COLOR RED-BLACK
    st.markdown('##### 25 cidades com mais restaurantes que realizam entregas')
    fig = city_delivery (df1)
    st.plotly_chart(fig, use_container_width=True)
###

with st.container():# gráfico de barras - COLOR RED-BLACK
    st.markdown('##### 25 cidades com maior variedade culinária')
...
with st.container():
    ...
    with col1: #grafico de barras 1/2 color = RED
        st.markdown('##### 5 cidades com maior média de avaliações recebidas')
        fig = best_5_cities (df1, 'votes')
        st.plotly_chart(fig, use_container_width=True)
###

    with col2: #grafico de barras 1/2 color = RED
        st.markdown('##### 5 cidades com melhor nota média')
        fig = best_5_cities (df1, 'aggregate_rating')
        st.plotly_chart(fig, use_container_width=True)
###
